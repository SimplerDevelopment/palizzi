      ...
      <div className="max-w-2xl mx-auto">
        ...
        {(hoursTitle || hoursSubtitle) && (
          <div className="text-center mb-16" style={{ border: '1px solid rgba(201,169,110,0.2)', padding: '2rem 1.5rem', backgroundColor: 'rgba(26,26,26,0.2)' }}>
            {hoursTitle && <p style={{ fontFamily: '"Playfair Display", Georgia, serif', fontSize: 'clamp(1.25rem, 3vw, 1.5rem)', color: '#c9a96e' }}>{hoursTitle}</p>}
            {hoursSubtitle && <p style={{ color: 'rgba(245,230,211,0.6)', marginTop: '0.5rem', letterSpacing: '0.05em' }}>{hoursSubtitle}</p>}
            {badges.length > 0 && (
              <div className="flex items-center justify-center gap-4" style={{ marginTop: '1rem' }}>
                {badges.map((badge, i) => (
                  <span key={i} className="flex items-center gap-4">
                    ...
                    <span style={{ fontSize: '0.75rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(245,230,211,0.4)' }}>{getText(badge)}</span>
                  </span>
                ))}
              </div>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {rules.map((rule, i) => (
              <div key={i} className="flex gap-4 items-start">
                ...
                <p style={{ color: 'rgba(245,230,211,0.7)', lineHeight: 1.75, borderBottom: i < rules.length - 1 ? '1px solid rgba(201,169,110,0.05)' : 'none', paddingBottom: '1.5rem' }}>{getText(rule)}</p>
              </div>
            ))}
          </div>
        )}
